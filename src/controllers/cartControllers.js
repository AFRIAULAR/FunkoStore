...
module.exports = {
    ...
    cart: async (req, res) => {
        try {
            ...
            const licence = await cartService.getLicence(1);
            
            res.render('store/cart', { items: items, info: info, licence: licence });
        } catch (error) {
            console.error('Error al obtener datos del carrito:', error);
            res.status(500).render('error', { message: 'Error al obtener datos del carrito.', error });
        }
    }
